...
export default function QuoteForm({ payload, mode }: { payload: DesignPayload; mode: "text" | "logo" }) {
  ...
  const finalPrice = useMemo(() => {
    const clean = payload.text.replace(/\s+/g, "");
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    // Optional mockup capture
    let mockupDataUrl = "";
    try {
      const node = document.getElementById("sign-preview");
      if (node) {
        mockupDataUrl = await htmlToImage.toJpeg(node as HTMLElement, { quality: 0.8, canvasWidth: 480, pixelRatio: 1 });
      }
    } catch {}
  ...
  }, [name, email, phone, message, payload, finalPrice]);

  return (
    <form onSubmit={handleSubmit} className="space-y-4 rounded-2xl border border-white/10 bg-white/5 p-4">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold tracking-wide text-white/80">Ready for a Quote?</h3>
        <div className="text-xs text-white/60">Estimated price: ${finalPrice}</div>
      </div>

      <div>
        <label htmlFor="q-name" className="mb-1 block text-xs text-white/70">Full name</label>
        <input
          id="q-name"
          value={name}
      ...
      <div>
        <label htmlFor="q-email" className="mb-1 block text-xs text-white/70">Email</label>
        <input
          id="q-email"
          value={email}
      ...
      <div>
        <label htmlFor="q-phone" className="mb-1 block text-xs text-white/70">Phone (optional)</label>
        <input
          id="q-phone"
          value={phone}
      ...
      <div>
        <label htmlFor="q-notes" className="mb-1 block text-xs text-white/70">Design notes (optional)</label>
        <textarea
          id="q-notes"
          value={message}
      ...
      {ok === true && <p className="text-sm text-emerald-300">Got it — we’ll email you back shortly.</p>}
